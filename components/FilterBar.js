  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition-colors dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="mt-4 border-t border-slate-200 pt-4 dark:border-slate-800">
        <button
          type="button"
          onClick={() => update({ compareEnabled: !filters.compareEnabled })}
          disabled={disabled}
          aria-pressed={filters.compareEnabled}
          className={[
            "inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-40",
            filters.compareEnabled
              ? "bg-velot text-white hover:bg-velot-dark"
        ...
        >
          ⇄ Comparar períodos {filters.compareEnabled ? "(ativo)" : ""}
        </button>
        ...
        {filters.compareEnabled && (
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <label className="flex flex-col gap-1.5">
              ...
              <select
                className={selectClass}
                value={filters.compareDateRange}
                disabled={disabled}
                onChange={(e) => update({ compareDateRange: e.target.value })}
              >
                {DATE_RANGE_OPTIONS.map((opt) => (
                  <option key={opt.value} value={opt.value}>
                    {opt.label}
            ...
            {filters.compareDateRange === "custom" && (
              <>
                <label className="flex flex-col gap-1.5">
                  <span className="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">
                    Comp. — De
                  ...
                  <input
                    type="date"
                    className={selectClass}
                    value={filters.compareCustomStart || ""}
                    disabled={disabled}
                    max={filters.compareCustomEnd || undefined}
                    onChange={(e) => update({ compareCustomStart: e.target.value })}
                  />
                </label>
                <label className="flex flex-col gap-1.5">
                  <span className="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">
                  ...
                  <input
                    type="date"
                    className={selectClass}
                    value={filters.compareCustomEnd || ""}
                    disabled={disabled}
                    min={filters.compareCustomStart || undefined}
                    onChange={(e) => update({ compareCustomEnd: e.target.value })}
                  />
                </label>
              </>
            )}
