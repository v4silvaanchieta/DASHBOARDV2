...
export function computeStoreHygiene(filteredData) {
  ...
  return Array.from(map.values())
    .map((entry) => {
      // Score Gamificado com DUPLA TRAVA (o erro sempre pune):
      const estagnados = entry.estagnados;
      const movimentados = entry.totalLeads - estagnados; // tudo que avançou/trabalhou

      // Passo 1: teto base (apenas ganhos) — sobe 2 por movimentado, teto 100.
      const placarBase = Math.min(100, 50 + movimentados * 2);
      // Passo 2: punição soberana (-5 por estagnado) aplicada DEPOIS do teto.
      const score = Math.max(0, Math.round(placarBase - estagnados * 5));
      return { ...entry, score };
    })
