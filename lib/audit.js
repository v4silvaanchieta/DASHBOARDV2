 ...
 * + Ganhos + Perdidos (reconcilia com Total Leads) + flags de higiene + Score.
...
export function computeStoreReport(filteredData, penalties = SCORE_PENALTY) {
  ...
  // Visão CRM completa que reconcilia com o Total de Leads:
  //   funil de cards ABERTOS por estágio + Ganhos + Perdidos = total da loja.
  ...
  for (const row of filteredData) {
    ...
    if (isGanho(row.status)) {
      entry.ganhos += 1;
    } else if (isPerdido(row.status)) {
      entry.perdidos += 1;
    } else {
      // Card aberto -> distribui no funil pelo estágio atual.
      const stage = classifyStage(row.estagio);
      if (stage) {
        entry[stage] += 1;
        // Estagnados: regra única (1ª fase + 48h) — só cards realmente parados.
