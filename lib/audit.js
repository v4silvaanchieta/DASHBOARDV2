...
/**
 * Fases abertas que compõem a BASE UNIVERSAL ATIVA (Total de Leads Ativos):
 * Pré-Qualificação + Novos Leads + Triagem + Análise + Faturamento.
 */
...
export const SCORE_WEIGHTS = {
  agilidade: 40, // estagnados / total de leads ativos (MAIOR PESO)
  propriedade: 20, // cards "TI BH" / total de leads ativos
  faturamento: 20, // ganhos zerados / total de ganhos
  auditoriaPerdas: 20, // perdas sem motivo / total de perdas
};
...
export function computeStoreHygiene(filteredData) {
  ...
  for (const row of filteredData) {
    ...
    // Base universal ativa (pilares 1 e 2) + bases de ganho/perda (pilares 3 e 4).
    if (stageHas(row.estagio, ACTIVE_FUNNEL_STAGES)) entry.totalAtivos += 1;
  ...
  return Array.from(map.values())
    .map(({ totalAtivos, ganhoBase, perdidoBase, ...rest }) => {
      // Pilares 1 e 2 usam o Total de Leads Ativos como denominador.
      const p1 = pillarScore(SCORE_WEIGHTS.agilidade, rest.estagnados, totalAtivos);
      const p2 = pillarScore(SCORE_WEIGHTS.propriedade, rest.tiBh, totalAtivos);
