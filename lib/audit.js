...
export function computeStoreHygiene(filteredData) {
  ...
  return Array.from(map.values())
    .map((entry) => {
      // Índice de Eficiência de SLA (estilo NPS): cabo de guerra entre acertos
      // (movimentados) e negligências (estagnados), partindo do neutro (50).
      const estagnados = entry.estagnados;
      const total = entry.totalLeads; // movimentados + estagnados
      const movimentados = total - estagnados;

      let score = 50; // ponto neutro (ausência de dados)
      if (total > 0) {
        const taxa = (movimentados - estagnados) / total; // -1 .. +1
        score = Math.max(0, Math.min(100, Math.round(50 + taxa * 50)));
      }
      return { ...entry, score };
    })
