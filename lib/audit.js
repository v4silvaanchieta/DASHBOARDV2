...
/**
 * Matriz analítica por loja (aba Relatórios): distribuição de TODOS os cards por
 * estágio (reconcilia com Total Leads) + flags de higiene + Score.
...
export function computeStoreReport(filteredData, penalties = SCORE_PENALTY) {
  ...
  // Distribuição de TODOS os cards por estágio (abertos + ganhos + perdidos),
  // para que a soma das colunas reconcilie com o Total de Leads da loja.
  // (Estagnados continua sendo um subconjunto: aberto + Pré-Qualificação + >48h.)
  ...
  for (const row of filteredData) {
    ...
    // Estagnados: regra única (1ª fase + 48h) — só conta cards realmente parados.
