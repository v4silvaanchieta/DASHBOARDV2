...
import { FRONTLINE_CITIES } from "@/lib/filters";
...
/**
 * BARREIRA (Linha de Frente): true se o Adset Name pertence a QUALQUER cidade da
 * lista de lojas com campanhas ativas (FRONTLINE_CITIES). Casa por substring
 * normalizada, mesma regra do match por pipeline.
 *
 * @param {string} adsetName
 * @returns {boolean}
 */
export function campaignMatchesFrontline(adsetName) {
  const a = norm(adsetName);
  if (!a) return false;
  for (const city of FRONTLINE_CITIES) {
    if (a.includes(city)) return true;
  }
  return false;
}
