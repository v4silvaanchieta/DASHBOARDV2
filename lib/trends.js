...
/**
 * Janela de análise CONSOLIDADA: de "Ontem − N dias" até "Ontem" (inclusive).
 * O dia de HOJE é excluído (dados parciais/não consolidados) — por isso `end`
 * é EXCLUSIVO e igual à meia-noite de hoje (tudo com data < hoje = até ontem).
 *
 * @param {"30d"|"3m"|"1y"} range
 * @param {Date} [now=new Date()]
 * @returns {{ start: Date, end: Date }}  start inclusivo · end exclusivo (hoje)
 */
export function trendWindow(range, now = new Date()) {
  const days = RANGE_DAYS[range] ?? 30;
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const start = new Date(today);
  start.setDate(start.getDate() - 1 - days); // ontem − N dias (início inclusivo)
  return { start, end: today }; // end exclusivo -> exclui HOJE (janela até ontem)
}
...
/**
 * Dias EXCLUÍDOS das Tendências: datas atípicas (ex.: dia sem veiculação de
 * anúncio) que viram outlier e distorcem a escala dos sparklines. Formato
 * "YYYY-MM-DD"; o dia some de TODAS as séries (o traço apenas liga por cima).
 */
export const EXCLUDED_TREND_DATES = new Set([
  "2026-09-04", // sem anúncio no ar -> Qualificação ~900% (outlier visual)
]);
...
export function generateTrendData(
  ...
  const { start, end } = trendWindow(range, now);
  const buckets = new Map();

  // Retorna o bucket do dia da data (ou null se fora da janela / data inválida).
  // `d >= end` exclui HOJE (dia não consolidado); só entra até ontem.
  const at = (rawDate) => {
    const d = parseDate(rawDate);
    if (!d || d < start || d >= end) return null;
    const key = toYMD(d);
    if (EXCLUDED_TREND_DATES.has(key)) return null; // dia atípico removido
  ...
  // Deals (CRM): entradas pela DATA DE CRIAÇÃO; ganhos pela DATA DO GANHO
  // (dataAtualizacao) — consistente com o funil/CRM. Sem data de ganho, cai na
  // criação.
  for (const d of deals) {
    const b = at(d.dataCriacao);
    if (b) b.deals += 1;
    if (norm(d.status) === "ganho") {
      const bg = at(d.dataAtualizacao) || b;
      if (bg) bg.ganhos += 1;
    }
  }
  ...
  return Array.from(buckets.values())
    .map((b) => ({
      ...
      // Séries ABSOLUTAS (quantidade) — novos sparklines.
      conversas: b.conversas,
      qualificadosCrm: b.deals,
      ganhos: b.ganhos,
    }))
...
export function computeTrendSummary(
  ...
  const { start, end } = trendWindow(range, now);
  // Exclui HOJE (d >= end): médias ponderadas só sobre dias consolidados (até ontem).
  const within = (rawDate) => {
    const d = parseDate(rawDate);
    return (
      Boolean(d) && d >= start && d < end && !EXCLUDED_TREND_DATES.has(toYMD(d))
    );
  };
  ...
  for (const d of deals) {
    if (within(d.dataCriacao)) dealsCount += 1;
    // Ganhos totais: pela DATA DO GANHO (dataAtualizacao); sem ela, pela criação.
    if (norm(d.status) === "ganho") {
      const wonRef = parseDate(d.dataAtualizacao) ? d.dataAtualizacao : d.dataCriacao;
      if (within(wonRef)) ganhos += 1;
    }
  }
  ...
  return {
    ...
    // Totais ABSOLUTOS (quantidade) — números ao lado dos novos sparklines.
    conversas,
    qualificadosCrm: dealsCount,
    ganhos,
  };
}
