...
export const SOURCE_URLS = {
  ...
  // Aba RELATÓRIO DE VENDAS — vendas reais das lojas (fonte de verdade do volume
  // vendido). Colunas: Vendedor, Modelo, Valor da venda, Nome do cliente, Telefone,
  // CPF, Data. Cruzada com o CRM para identificar origem V4 vs desconhecida.
  vendas:
...
/**
 * Converte o "Valor da venda" (formato do relatório, ex.: "14490.00" com PONTO
 * decimal, ou eventual "14.490,00" BR) para Number. Diferente de parseQuantia,
 * que trata ponto como separador de milhar.
 *
...
export function parseSaleValue(value) {
  ...
  // Se tem vírgula, é BR (ponto = milhar, vírgula = decimal). Senão, ponto = decimal.
...
/**
 * Busca a aba RELATÓRIO DE VENDAS já normalizada.
 * @returns {Promise<Array<Record<string, any>>>}
 */
 ...
 *   campanhas: Array<Record<string, any>>,
 *   vendas: Array<Record<string, any>>,
 * }>}
 */
export function fetchAllSources() {
  return Promise.all([
    ...
    fetchCampanhas(),
    fetchVendas(),
  ]).then(([deals, movimentacao, leadsSdr, campanhas, vendas]) => ({
    deals,
    movimentacao,
    leadsSdr,
    campanhas,
