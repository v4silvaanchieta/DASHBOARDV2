import Papa from "papaparse";

/**
 * URLs das 3 abas publicadas (output=csv) da planilha da Velot.
 * Cada aba é exportada por um link individual (Google Sheets exporta 1 aba por link).
 */
export const SOURCE_URLS = {
  // Aba LEADS SDR — entrada bruta de leads (colunas: DATA, TELEFONE, NOME).
  leadsSdr:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSfANI6fzYqt3yWoXMkSFVt0E1pfZBeiPKp2m3nQnoEC2HUWrfBu-Vqx4kZxFvjWFN7EmSzFHpn6b6o/pub?gid=917398745&single=true&output=csv",
  // Aba MOVIMENTAÇÃO — deals (mesmo formato de 41 colunas, datas sem hora).
  movimentacao:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSfANI6fzYqt3yWoXMkSFVt0E1pfZBeiPKp2m3nQnoEC2HUWrfBu-Vqx4kZxFvjWFN7EmSzFHpn6b6o/pub?gid=0&single=true&output=csv",
  // Aba DEALS — deals consolidados (41 colunas, datas com hora).
  deals:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSfANI6fzYqt3yWoXMkSFVt0E1pfZBeiPKp2m3nQnoEC2HUWrfBu-Vqx4kZxFvjWFN7EmSzFHpn6b6o/pub?gid=1920849068&single=true&output=csv",
  // Aba CAMPANHAS — tráfego pago (Meta Ads). A cidade/unidade está no Adset Name.
  campanhas:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSfANI6fzYqt3yWoXMkSFVt0E1pfZBeiPKp2m3nQnoEC2HUWrfBu-Vqx4kZxFvjWFN7EmSzFHpn6b6o/pub?gid=2145994455&single=true&output=csv",
  // Aba MENU — de-para oficial Vendedor(ID/nome do sistema) -> Loja (Velot | Cidade).
  menu:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSfANI6fzYqt3yWoXMkSFVt0E1pfZBeiPKp2m3nQnoEC2HUWrfBu-Vqx4kZxFvjWFN7EmSzFHpn6b6o/pub?gid=152635867&single=true&output=csv",
  // Aba RELATÓRIO DE VENDAS — vendas reais (banco + à vista + CDCI) por vendedor.
  vendas:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSfANI6fzYqt3yWoXMkSFVt0E1pfZBeiPKp2m3nQnoEC2HUWrfBu-Vqx4kZxFvjWFN7EmSzFHpn6b6o/pub?gid=1267714554&single=true&output=csv",
  // Aba FUNIL DIÁRIO — funil COMERCIAL por dia/loja: consultas, elegíveis, propostas, vendas.
  funilDiario:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSfANI6fzYqt3yWoXMkSFVt0E1pfZBeiPKp2m3nQnoEC2HUWrfBu-Vqx4kZxFvjWFN7EmSzFHpn6b6o/pub?gid=2107280473&single=true&output=csv",
};

/** Mantido por compatibilidade: a fonte primária do dashboard é a aba DEALS. */
export const CSV_URL = SOURCE_URLS.deals;

/**
 * Mapeamento: cabeçalho da aba DEALS/MOVIMENTAÇÃO -> chave do objeto JSON.
 */
export const COLUMN_MAP = {
  "DATA CRIAÇÃO": "dataCriacao",
  "DATA ATUALIZAÇÃO": "dataAtualizacao",
  PIPELINE: "pipeline",
  "ESTÁGIO": "estagio",
  "DEAL ID": "dealId",
  "NOME DEAL": "nomeDeal",
  STATUS: "status",
  "MOTIVO DE PERDA": "motivoPerda",
  "QUANTIDADE PRODUTOS": "quantidadeProdutos",
  PRODUTOS: "produtos",
  QUANTIA: "quantia",
  "DATA PREVISTA FECHAMENTO": "dataPrevistaFechamento",
  TAGS: "tags",
  "PROPRIETÁRIO": "proprietario",
  "NOME CONTATO": "nomeContato",
  TELEFONE: "telefone",
  EMAIL: "email",
  CPF: "cpf",
  CF_TELEFONE: "cfTelefone",
  CF_CIDADE: "cidade",
  CF_FORMA_DE_PAGAMENTO: "formaPagamento",
  CF_PRODUTO_DEFINIDO: "produtoDefinido",
  CF_UTM_SOURCE: "utmSource",
  CF_UTM_MEDIUM: "utmMedium",
  CF_UTM_CAMPAIGN: "utmCampaign",
  CF_UTM_CONTENT: "utmContent",
  CF_UTM_TERM: "utmTerm",
  CF_LINK_WPP: "linkWpp",
};

/** Mapeamento da aba LEADS SDR. */
export const LEADS_SDR_MAP = {
  DATA: "data",
  TELEFONE: "telefone",
  NOME: "nome",
};

/** Mapeamento da aba MENU (de-para vendedor -> loja). */
export const MENU_MAP = {
  "Vendedor [ID_velot]": "vendedorId",
  Loja: "loja",
};

/** Mapeamento da aba RELATÓRIO DE VENDAS. */
export const VENDAS_MAP = {
  Vendedor: "vendedor",
  Modelo: "modelo",
  "Nome do cliente": "nomeCliente",
  Telefone: "telefone",
  CPF: "cpf",
  Data: "data",
};

/** Mapeamento da aba FUNIL DIÁRIO (funil comercial por dia/loja). */
export const FUNIL_DIARIO_MAP = {
  Data: "data",
  Vendedor: "vendedor",
  "Consulta totais": "consultas",
  "CPF Elegiveis": "elegiveis",
  "Propostas enviadas": "propostas",
  Vendas: "vendas",
};

/** Converte "R$ 19.990,00" | "14490.00" | 14990 -> Number. */
export function parseSaleValue(value) {
  if (value === null || value === undefined || value === "") return 0;
  if (typeof value === "number") return Number.isFinite(value) ? value : 0;
  let s = String(value).replace(/R\$/gi, "").replace(/\s/g, "").replace(/[^\d.,-]/g, "");
  if (s.includes(",")) s = s.replace(/\./g, "").replace(",", "."); // BR: ponto=milhar, vírgula=decimal
  const n = parseFloat(s);
  return Number.isNaN(n) ? 0 : n;
}

/** Converte texto para inteiro seguro (funil diário). */
function parseInt0(value) {
  const n = parseInt(String(value ?? "").replace(/[^\d-]/g, ""), 10);
  return Number.isNaN(n) ? 0 : n;
}

/**
 * Mapeamento da aba CAMPANHAS (tráfego pago / Meta Ads).
 * Os cabeçalhos têm parênteses descritivos — casados exatamente como na planilha.
 */
export const CAMPAIGNS_MAP = {
  Date: "date",
  "Spend (Cost, Amount Spent)": "spend",
  "Campaign Name": "campaignName",
  "Adset Name": "adsetName",
  "Ad Name": "adName",
  Impressions: "impressions",
  Clicks: "clicks",
  "CPM (Cost per 1000 Impressions)": "cpm",
  "CPC (Cost per Click)": "cpc",
  "CTR (Clickthrough Rate)": "ctr",
  "Action Messaging Conversations Started (Onsite Conversion)": "conversations",
};

/** Campos numéricos da aba Campanhas — limpos via parseQuantia (BR/R$/milhar). */
const CAMPAIGN_NUMERIC_KEYS = [
  "spend",
  "impressions",
  "clicks",
  "cpm",
  "cpc",
  "ctr",
  "conversations",
];

/**
 * Converte um valor monetário (BR ou já numérico) para Number.
 * Ex.: "R$ 1.234,56" -> 1234.56 | 17991 -> 17991 | "" -> 0
 *
 * @param {string|number} value
 * @returns {number}
 */
export function parseQuantia(value) {
  if (value === null || value === undefined || value === "") return 0;
  if (typeof value === "number") return Number.isFinite(value) ? value : 0;

  let normalized = String(value)
    .replace(/R\$/gi, "")
    .replace(/\s/g, "")
    .replace(/[^\d,.-]/g, "");

  normalized = normalized.replace(/\./g, "").replace(",", ".");

  const parsed = parseFloat(normalized);
  return Number.isNaN(parsed) ? 0 : parsed;
}

/**
 * Acrescenta um parâmetro de tempo à URL para forçar a quebra de cache.
 * As URLs já contêm "?", então o separador é sempre "&".
 *
 * @param {string} url
 * @returns {string}
 */
export function withCacheBuster(url) {
  const separator = url.includes("?") ? "&" : "?";
  return `${url}${separator}t=${new Date().getTime()}`;
}

/**
 * Reindexa uma linha bruta por cabeçalho "trimado", tolerando espaços ocultos.
 * @param {Record<string, any>} row
 * @returns {Record<string, any>}
 */
function trimKeys(row) {
  const trimmed = {};
  for (const key of Object.keys(row)) {
    if (typeof key === "string") trimmed[key.trim()] = row[key];
  }
  return trimmed;
}

/** Normaliza um valor de célula para string aparada. */
function cell(raw) {
  return raw === undefined || raw === null ? "" : String(raw).trim();
}

/** Mapeia uma linha de DEALS/MOVIMENTAÇÃO conforme COLUMN_MAP. */
function mapDealRow(row) {
  const trimmed = trimKeys(row);
  const mapped = {};
  for (const [csvHeader, jsonKey] of Object.entries(COLUMN_MAP)) {
    mapped[jsonKey] = cell(trimmed[csvHeader]);
  }
  // QUANTIA -> número (a partir do valor bruto, antes do .trim()).
  mapped.quantia = parseQuantia(trimmed["QUANTIA"]);
  return mapped;
}

/** Mapeia uma linha da aba LEADS SDR. */
function mapLeadRow(row) {
  const trimmed = trimKeys(row);
  const mapped = {};
  for (const [csvHeader, jsonKey] of Object.entries(LEADS_SDR_MAP)) {
    mapped[jsonKey] = cell(trimmed[csvHeader]);
  }
  return mapped;
}

/** Mapeia uma linha da aba MENU (de-para). */
function mapMenuRow(row) {
  const trimmed = trimKeys(row);
  return {
    vendedorId: cell(trimmed["Vendedor [ID_velot]"]),
    loja: cell(trimmed["Loja"]),
  };
}

/** Mapeia uma linha da aba RELATÓRIO DE VENDAS. */
function mapVendaRow(row) {
  const trimmed = trimKeys(row);
  const mapped = {};
  for (const [csvHeader, jsonKey] of Object.entries(VENDAS_MAP)) {
    mapped[jsonKey] = cell(trimmed[csvHeader]);
  }
  mapped.valorVenda = parseSaleValue(trimmed["Valor da venda"]);
  return mapped;
}

/** Mapeia uma linha da aba FUNIL DIÁRIO (números do funil comercial). */
function mapFunilDiarioRow(row) {
  const trimmed = trimKeys(row);
  return {
    data: cell(trimmed["Data"]),
    vendedor: cell(trimmed["Vendedor"]),
    consultas: parseInt0(trimmed["Consulta totais"]),
    elegiveis: parseInt0(trimmed["CPF Elegiveis"]),
    propostas: parseInt0(trimmed["Propostas enviadas"]),
    vendas: parseInt0(trimmed["Vendas"]),
  };
}

/** Mapeia uma linha da aba CAMPANHAS, convertendo métricas para Float. */
function mapCampaignRow(row) {
  const trimmed = trimKeys(row);
  const mapped = {};
  for (const [csvHeader, jsonKey] of Object.entries(CAMPAIGNS_MAP)) {
    mapped[jsonKey] = cell(trimmed[csvHeader]);
  }
  // Métricas financeiras/numéricas -> Float (limpa "R$", separador de milhar com
  // ponto e decimal com vírgula, ex.: "R$ 1.234,56" -> 1234.56 | "0,03" -> 0.03).
  for (const key of CAMPAIGN_NUMERIC_KEYS) {
    mapped[key] = parseQuantia(mapped[key]);
  }
  return mapped;
}

/**
 * Faz o parse de um CSV remoto com PapaParse e devolve as linhas brutas.
 *
 * @param {string} url
 * @param {string} label  Rótulo para o log de debug.
 * @returns {Promise<Array<Record<string, any>>>}
 */
function parseCsv(url, label) {
  const bustedUrl = withCacheBuster(url);
  return new Promise((resolve, reject) => {
    Papa.parse(bustedUrl, {
      download: true,
      header: true,
      skipEmptyLines: true,
      dynamicTyping: true,
      transformHeader: (header) => (typeof header === "string" ? header.trim() : header),
      complete: (results) => {
        // Debug solicitado: inspecionar o retorno do PapaParse.
        console.log(`Dados recebidos do PapaParse [${label}]:`, results.data);
        resolve(results.data || []);
      },
      error: (err) => reject(err),
    });
  });
}

/**
 * Busca a aba DEALS já normalizada.
 * @returns {Promise<Array<Record<string, any>>>}
 */
export function fetchDeals() {
  return parseCsv(SOURCE_URLS.deals, "DEALS").then((rows) =>
    rows.map(mapDealRow).filter((r) => r.dealId !== "" || r.nomeDeal !== "")
  );
}

/**
 * Busca a aba MOVIMENTAÇÃO já normalizada (mesmo formato de DEALS).
 * @returns {Promise<Array<Record<string, any>>>}
 */
export function fetchMovimentacao() {
  return parseCsv(SOURCE_URLS.movimentacao, "MOVIMENTACAO").then((rows) =>
    rows.map(mapDealRow).filter((r) => r.dealId !== "" || r.nomeDeal !== "")
  );
}

/**
 * Busca a aba LEADS SDR já normalizada.
 * @returns {Promise<Array<Record<string, any>>>}
 */
export function fetchLeadsSdr() {
  return parseCsv(SOURCE_URLS.leadsSdr, "LEADS_SDR").then((rows) =>
    rows.map(mapLeadRow).filter((r) => r.telefone !== "" || r.nome !== "")
  );
}

/**
 * Busca a aba CAMPANHAS já normalizada (métricas como Float).
 * @returns {Promise<Array<Record<string, any>>>}
 */
export function fetchCampanhas() {
  return parseCsv(SOURCE_URLS.campanhas, "CAMPANHAS").then((rows) =>
    rows
      .map(mapCampaignRow)
      .filter((r) => r.date !== "" || r.campaignName !== "" || r.adsetName !== "")
  );
}

/** Busca a aba MENU (de-para vendedor -> loja). */
export function fetchMenu() {
  return parseCsv(SOURCE_URLS.menu, "MENU").then((rows) =>
    rows.map(mapMenuRow).filter((r) => r.vendedorId !== "" || r.loja !== "")
  );
}

/** Busca a aba RELATÓRIO DE VENDAS. */
export function fetchVendas() {
  return parseCsv(SOURCE_URLS.vendas, "VENDAS").then((rows) =>
    rows.map(mapVendaRow).filter((r) => r.nomeCliente !== "" || r.vendedor !== "")
  );
}

/** Busca a aba FUNIL DIÁRIO (funil comercial). */
export function fetchFunilDiario() {
  return parseCsv(SOURCE_URLS.funilDiario, "FUNIL_DIARIO").then((rows) =>
    rows.map(mapFunilDiarioRow).filter((r) => r.vendedor !== "" && r.data !== "")
  );
}

/**
 * Busca todas as abas em paralelo (Promise.all) e devolve o estado unificado.
 *
 * @returns {Promise<{
 *   deals, movimentacao, leadsSdr, campanhas, menu, vendas, funilDiario
 * }>}
 */
export function fetchAllSources() {
  return Promise.all([
    fetchDeals(),
    fetchMovimentacao(),
    fetchLeadsSdr(),
    fetchCampanhas(),
    fetchMenu(),
    fetchVendas(),
    fetchFunilDiario(),
  ]).then(([deals, movimentacao, leadsSdr, campanhas, menu, vendas, funilDiario]) => ({
    deals,
    movimentacao,
    leadsSdr,
    campanhas,
    menu,
    vendas,
    funilDiario,
  }));
}

/**
 * Compatibilidade: retorna apenas a aba DEALS (fonte primária do dashboard).
 * @param {string} [url=CSV_URL]
 * @returns {Promise<Array<Record<string, any>>>}
 */
export function fetchData() {
  return fetchDeals();
}
