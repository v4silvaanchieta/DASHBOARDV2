...
/** Filtro especial: apenas as lojas da "Linha de Frente" (campanhas ativas). */
export const PIPELINE_FRONTLINE = "__FRONTLINE__";
export const SOURCE_ALL = "__ALL_SOURCES__";

/**
 * "Linha de Frente" = lojas com campanhas de tráfego ATIVAS. Casadas pela CIDADE
 * (minúscula, sem acento), pois no CRM a pipeline é "Velot <Cidade>" enquanto no
 * tráfego a unidade vem como "Velot Motors | <Cidade>". Manter só a cidade evita
 * divergência de prefixo/acentuação entre as duas fontes.
 */
export const FRONTLINE_CITIES = new Set([
  "sapucaia do sul",
  "campo largo",
  "criciuma",
  "joacaba",
...
/** Cidade normalizada de uma pipeline: "Velot Barra Mansa" -> "barra mansa". */
export function cityKeyFromPipeline(pipeline) {
...
/** true se a pipeline pertence à Linha de Frente (campanhas ativas). */
export function isFrontlinePipeline(pipeline) {
  return FRONTLINE_CITIES.has(cityKeyFromPipeline(pipeline));
}
/** Origem especial: deals com link de WhatsApp (CF_LINK_WPP) ou tag "V4". */
export const SOURCE_V4 = "__SOURCE_V4__";
/** Origem especial: vendas do relatório SEM match no CRM (origem não identificada). */
export const SOURCE_UNKNOWN = "__SOURCE_UNKNOWN__";
 ...
 * @param {string} [dateField="dataCriacao"]  Coluna de data usada no filtro de
 *   período. Ex.: "dataAtualizacao" para atribuir vendas ganhas à data do ganho.
 * @returns {Array<Record<string, any>>}
 */
export function applyFilters(data, filters, dateField = "dataCriacao") {
  const { dateRange, pipeline, source, customStart, customEnd } = filters;
  const custom = { start: customStart, end: customEnd };

  return data.filter((row) => {
    // Filtro de data. Padrão = DATA CRIAÇÃO; pode ser DATA ATUALIZAÇÃO (data do
    // ganho). Cai para a criação se o campo escolhido estiver vazio.
    if (dateRange && dateRange !== "all") {
      const ref = parseDate(row[dateField]) || parseDate(row.dataCriacao);
      if (!isWithinRange(ref, dateRange, custom)) return false;
    }
    ...
    } else if (pipeline === PIPELINE_FRONTLINE) {
      // "Linha de Frente" = subconjunto de lojas com campanhas ativas (por cidade).
      if (!isFrontlinePipeline(row.pipeline)) return false;
    } else if (pipeline) {
      // Seleção específica (ex.: "Matriz" ou uma franquia) -> match exato.
      if (String(row.pipeline).trim() !== pipeline) return false;
    }

    // Filtro de Origem (coluna CF_UTM_SOURCE -> utmSource, ou origens especiais).
    if (source && source !== SOURCE_ALL) {
      if (source === SOURCE_V4) {
        if (!isV4Lead(row)) return false;
      } else if (source === SOURCE_UNKNOWN) {
        // Só vendas do relatório sem match no CRM.
        if (!row.origemDesconhecida) return false;
