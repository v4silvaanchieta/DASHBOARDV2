...
/**
 * true se o lead está AGUARDANDO o primeiro atendimento: card ABERTO e ainda na
 * etapa inicial de Pré-Qualificação (SDR / IA) — ou seja, acabou de entrar e não
 * foi movido. Base da "Fila de Atendimento" (radar de gargalo em tempo real).
...
export function isAwaitingSdrLead(row) {
  return isOpenStatus(row.status) && stageIn(row, INITIAL_STAGES);
}
