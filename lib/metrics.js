...
/** Estágios considerados "etapa inicial" (lead ainda aguardando atendimento). */
export const INITIAL_STAGES = ["PRÉ-QUALIFICAÇÃO", "SDR IA"];

/** Estágios que indicam que o lead já avançou para atendimento. */
export const ATTENDED_STAGES = ["EM ATENDIMENTO", "NOVOS LEADS"];
...
/** true se o estágio do lead está em uma das listas informadas. */
function stageIn(row, stages) {
  const estagio = norm(row.estagio);
  return stages.some((s) => norm(s) === estagio);
}
...
export function computeMetrics(filteredData) {
  ...
  const acc = filteredData.reduce(
    (a, row) => {
      ...
      // Leads parados: na etapa inicial há mais de 24h (DATA CRIAÇÃO vs agora).
      if (stageIn(row, INITIAL_STAGES)) {
        const created = parseDate(row.dataCriacao);
        if (created && now - created.getTime() > HOURS_24_MS) {
          a.leadsParados += 1;
        }
      }
 ...
 * }}
 */
export function computeSLA(filteredData) {
  let totalMinutes = 0;
  let sampleSize = 0;
  let waitingCount = 0;
  ...
  return {
    ...
    sampleSize,
    waitingCount,
    withinTarget: sampleSize > 0 && avgMinutes < SLA_TARGET_MINUTES,
  };
}
